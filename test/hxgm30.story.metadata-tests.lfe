...
(deftest toml->map
  (let* ((toml (hxgm30.story.metadata:read (test-path) "metadata.toml"))
         (data (hxgm30.story.metadata:toml->map toml)))
...
(deftest toml->map-deeply-nested
  (let* ((toml (hxgm30.story.metadata:read (test-path) "deep-sections.toml"))
         (data (hxgm30.story.metadata:toml->map toml)))
    (is-equal 42
              (mref data "ab"))
    (is-equal 3.14159
